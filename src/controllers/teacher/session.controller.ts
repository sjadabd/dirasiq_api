...
export class TeacherSessionController {
  ...
  static async createSession(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (!teacher || teacher.id !== req.body.teacher_id) {
        res.status(403).json({ success: false, message: 'غير مسموح', errors: ['صلاحيات غير كافية'] });
        return;
      }
      ...
      for (const k of required) {
        if (payload[k] === undefined || payload[k] === null) {
          res.status(400).json({ success: false, message: 'بيانات ناقصة', errors: [`حقل ${k} مطلوب`] });
          return;
        }
      }
      if (!hasWeekdaysArray && (payload.weekday === undefined || payload.weekday === null)) {
        res.status(400).json({ success: false, message: 'بيانات ناقصة', errors: ['weekday مطلوب عند عدم إرسال weekdays[]'] });
        return;
      }

      // Basic validations
      const validateWeekday = (d: any) => Number.isInteger(d) && d >= 0 && d <= 6;
      if (hasWeekdaysArray) {
        const invalid = payload.weekdays.some((d: any) => !validateWeekday(Number(d)));
        if (invalid) {
          res.status(400).json({ success: false, message: 'قيمة يوم الأسبوع غير صحيحة', errors: ['weekdays يجب أن تحتوي قيم بين 0 و 6'] });
          return;
        }
      } else {
        const weekday = Number(payload.weekday);
        if (!validateWeekday(weekday)) {
          res.status(400).json({ success: false, message: 'قيمة يوم الأسبوع غير صحيحة', errors: ['weekday يجب أن يكون بين 0 و 6'] });
          return;
        }
      }
      const timeRe = /^\d{2}:\d{2}(:\d{2})?$/;
      if (!timeRe.test(payload.start_time) || !timeRe.test(payload.end_time)) {
        res.status(400).json({ success: false, message: 'صيغة الوقت غير صحيحة', errors: ['start_time/end_time يجب أن تكون HH:MM أو HH:MM:SS'] });
        return;
      }
      const toSec = (s: string) => {
        const [h, m, sec] = String(s).split(':').map((x) => parseInt(x, 10));
        return (h || 0) * 3600 + (m || 0) * 60 + (sec || 0);
      };
      if (toSec(payload.start_time) >= toSec(payload.end_time)) {
        res.status(400).json({ success: false, message: 'الوقت غير صحيح', errors: ['start_time يجب أن يكون قبل end_time'] });
        return;
      }
      ...
      if (hasWeekdaysArray) {
        ...
        for (const dRaw of payload.weekdays) {
          ...
          const unique = Array.from(new Set<string>([...confirmed, ...provided].map(String)));
          if (unique.length > 0) {
          ...
          try {
            ...
            if (studentIds.length > 0) {
              await notifService.createAndSendNotification({
                ...
                message: `تمت إضافة جلسات جديدة لأيام: ${created.map((s) => s.weekday).join(', ')} من ${payload.start_time} إلى ${payload.end_time}`,
                type: NotificationType.COURSE_UPDATE,
                priority: NotificationPriority.HIGH,
                recipientType: RecipientType.SPECIFIC_STUDENTS,
                recipientIds: studentIds,
                data: { courseId: payload.course_id, sessionIds: created.map((s) => s.id) },
        ...
        res.status(201).json({ success: true, message: 'تم إنشاء الجلسات', data: { created, skipped } });
        return;
      }
      ...
      if (conflict) {
        res.status(409).json({ success: false, message: 'تعارض في الجدول', errors: ['يوجد جلسة أخرى متداخلة لنفس المعلم في نفس اليوم'] });
        return;
      }
      ...
      try {
        ...
        if (studentIds.length > 0) {
          await notifService.createAndSendNotification({
            title: 'تم إنشاء جدول الدروس',
            message: `تم إضافة جلسة جديدة: ${session.title || ''} يوم ${weekday} من ${session.start_time} إلى ${session.end_time}`.trim(),
            type: NotificationType.COURSE_UPDATE,
            priority: NotificationPriority.HIGH,
            recipientType: RecipientType.SPECIFIC_STUDENTS,
            recipientIds: studentIds,
      ...
      res.status(201).json({ success: true, message: 'تم إنشاء الجلسة', data: session });
    } catch (error) {
  ...
  static async addAttendees(req: Request<{ id: string }>, res: Response): Promise<void> {
    try {
      const sessionId = req.params['id'];
      const { studentIds } = req.body || {};

      if (!Array.isArray(studentIds) || studentIds.length === 0) {
        res.status(400).json({ success: false, message: 'قائمة الطلاب مطلوبة', errors: ['studentIds مطلوب'] });
        return;
      }
      ...
      res.status(200).json({ success: true, message: 'تم إضافة الطلاب إلى الجلسة' });
    } catch (error) {
  ...
  static async listAttendees(req: Request<{ id: string }>, res: Response): Promise<void> {
    try {
      const sessionId = req.params['id'];
      const session = await SessionModel.getById(sessionId);
      if (!session || session.teacher_id !== req.user?.id) {
      ...
      const ids = await SessionModel.listAttendeeIds(sessionId);
      res.status(200).json({ success: true, message: 'تم جلب الطلاب', data: ids });
    } catch (error) {
  ...
  static async removeAttendees(req: Request<{ id: string }>, res: Response): Promise<void> {
    try {
      const sessionId = req.params['id'];
      const { studentIds } = req.body || {};
      if (!Array.isArray(studentIds) || studentIds.length === 0) {
      ...
      res.status(200).json({ success: true, message: 'تم حذف الطلاب من الجلسة', data: { removed } });
    } catch (error) {
  ...
  static async listMySessions(req: Request, res: Response): Promise<void> {
    try {
      ...
      const { sessions, total } = await SessionModel.getTeacherSessions(teacher.id, page, limit);
      res.status(200).json({
        success: true,
  ...
  static async updateSession(req: Request<{ id: string }>, res: Response): Promise<void> {
    try {
      const id = req.params['id'];
      const me = req.user;
      const existing = await SessionModel.getById(id);
      if (!existing || existing.teacher_id !== me?.id) {
        res.status(404).json({ success: false, message: 'الجلسة غير موجودة أو غير مسموح' });
        return;
      }
      ...
      if (payload.weekday !== undefined) {
        ...
        if (!Number.isInteger(d) || d < 0 || d > 6) {
          res.status(400).json({ success: false, message: 'قيمة يوم الأسبوع غير صحيحة' });
          return;
        }
      }
      const timeRe = /^\d{2}:\d{2}(:\d{2})?$/;
      if ((payload.start_time && !timeRe.test(payload.start_time)) || (payload.end_time && !timeRe.test(payload.end_time))) {
        res.status(400).json({ success: false, message: 'صيغة الوقت غير صحيحة' });
        return;
      }
      ...
      const toSec = (s: string) => { const [h, m, sec] = String(s).split(':').map((x) => parseInt(x, 10)); return (h||0)*3600 + (m||0)*60 + (sec||0); };
      ...
      if (conflict) {
        res.status(409).json({ success: false, message: 'تعارض في الجدول', errors: ['يوجد جلسة أخرى متداخلة'] });
        return;
      }

      const updated = await SessionModel.updateSession(id, payload);
      res.status(200).json({ success: true, message: 'تم تحديث الجلسة', data: updated });
    } catch (error) {
  ...
  static async deleteSession(req: Request<{ id: string }>, res: Response): Promise<void> {
    try {
      const id = req.params['id'];
      const me = req.user;
      const existing = await SessionModel.getById(id);
      ...
      res.status(ok ? 200 : 404).json({ success: ok, message: ok ? 'تم حذف الجلسة' : 'الجلسة غير موجودة' });
    } catch (error) {
  ...
  static async endSessionAndNotify(req: Request<{ id: string }>, res: Response): Promise<void> {
    try {
      const sessionId = req.params['id'];
      const me = req.user;
      const session = await SessionModel.getById(sessionId);
      ...
      try {
        if (studentIds.length > 0) {
          ...
          await notifService.createAndSendNotification({
            ...
            recipientType: RecipientType.SPECIFIC_STUDENTS,
            recipientIds: studentIds,
            data: { sessionId: session.id, courseId: session.course_id, teacherId: session.teacher_id, endedAt: new Date().toISOString() },
      ...
      res.status(200).json({ success: true, message: 'تم إنهاء الجلسة وإشعار الطلاب' });
    } catch (error) {
      console.error('Error ending session:', error);
      res.status(500).json({ success: false, message: 'خطأ داخلي في الخادم', errors: ['تعذر إنهاء الجلسة'] });
    }
  }
}
