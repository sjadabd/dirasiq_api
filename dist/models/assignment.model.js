...
class AssignmentModel {
    ...
    `;
        ...
        const r = await database_1.default.query(q, vals);
        return r.rows[0];
    }
    ...
    static async softDelete(id) {
        const r = await database_1.default.query(`UPDATE assignments SET deleted_at = NOW() WHERE id = $1 AND deleted_at IS NULL`, [id]);
        return (r.rowCount ?? 0) > 0;
    }
    static async getById(id) {
        const r = await database_1.default.query(`SELECT * FROM assignments WHERE id = $1 AND deleted_at IS NULL`, [id]);
        return r.rows[0] || null;
    }
    static async listByTeacher(teacherId, page = 1, limit = 20, studyYear) {
        ...
        const r = await database_1.default.query(`SELECT * FROM assignments WHERE ${where} ORDER BY created_at DESC LIMIT $${params.length - 1} OFFSET $${params.length}`, params);
        ...
        const c = await database_1.default.query(`SELECT COUNT(*)::int AS c FROM assignments WHERE ${countWhere}`, countParams);
        return { data: r.rows, total: c.rows[0].c };
    }
    static async setRecipients(assignmentId, studentIds) {
    ...
    static async listForStudent(studentId, page = 1, limit = 20, studyYear) {
        const offset = (page - 1) * limit;
    ...
    `;
        ...
        const c = await database_1.default.query(`SELECT COUNT(*)::int AS c
    ...
    static async upsertSubmission(assignmentId, studentId, payload) {
        const q = `
    ...
    static async getSubmission(assignmentId, studentId) {
        const r = await database_1.default.query(`SELECT * FROM assignment_submissions WHERE assignment_id=$1 AND student_id=$2`, [assignmentId, studentId]);
        return r.rows[0] || null;
    }
    static async listSubmissionsByAssignment(assignmentId) {
        const r = await database_1.default.query(`SELECT * FROM assignment_submissions WHERE assignment_id = $1 ORDER BY submitted_at ASC, created_at ASC`, [assignmentId]);
        return r.rows;
    }
    static async gradeSubmission(assignmentId, studentId, score, gradedBy, feedback) {
        const r = await database_1.default.query(`UPDATE assignment_submissions SET score=$3, graded_at=NOW(), graded_by=$4, status='graded', feedback=COALESCE($5, feedback), updated_at=NOW()
       WHERE assignment_id=$1 AND student_id=$2 RETURNING *`, [assignmentId, studentId, score, gradedBy, feedback ?? null]);
        return r.rows[0] || null;
    }
    static async getRecipientIds(assignmentId) {
        const r = await database_1.default.query(`SELECT student_id FROM assignment_recipients WHERE assignment_id = $1`, [assignmentId]);
